...
export function Results() {
  ...
  function exportCsv() {
    const rows: string[] = [];
    rows.push(['施設・棟', '保険', '患者名', 'フリガナ', '訪問日', '区分', '備考'].join(','));
    for (const g of calc) {
        ...
        <div className="card" key={i}>
          <div className="card-head">
            <div>
              ...
              <div className="meta">
                {g.insurance}保険 ・ {ymToLabel(ym)}
              </div>
            </div>
            <div className="summary">
              <span>対象 <b>{g.rows.length}人</b></span>
              <span className="judge">区分: <b>{g.classification}</b></span>
            </div>
          </div>
